...
# START ================================
def DecomposeNodeActions(scene, obj, tData, tOptions):

    # TODO: refactor DecomposeActions() instead of duplicating most of its code
    ...
    if not obj.animation_data:
        log.warning('Object {:s} has no animation data'.format(obj.name))
        return

    originMatrix = Matrix.Identity(4)
    if tOptions.actionsGlobalOrigin:
        originMatrix = obj.matrix_world
        if tOptions.globalOrigin and originMatrix != Matrix.Identity(4):
            # Blender moves/rotates the armature together with the mesh, so if you set a global origin
            # for Mesh and Actions you'll have twice the transformations. Set only one global origin.
            log.warning("Use local origin for the object otherwise transformations are applied twice")
    ...
    # Save current action and frame, we'll restore them later
    savedAction = obj.animation_data.action
    savedFrame = scene.frame_current
    savedUseNla = obj.animation_data.use_nla
    ...
    if not animationObjects:
        log.warning('Object {:s} has no animation to export'.format(obj.name))
        return
    ...
    for object in animationObjects:
        ...
        if isinstance(object, bpy.types.Action):
            ...
            obj.animation_data.action = object
            # Get the Actions
            actionSet.add(object)
        ...
        if isinstance(object, bpy.types.Object):
            ...
            # If there are no Tracks use the saved action (NLA is empty so we can keep it on)
            if not object.animation_data.nla_tracks and savedAction:
                obj.animation_data.action = savedAction
                actionSet.add(savedAction)
            # Get the Actions
            for track in object.animation_data.nla_tracks:
                for strip in track.strips:
        ...
        # Progress counter
        progressCur = 0
        progressTot = 1 # 0.01 * len(bones) * (endframe-startframe)/scene.frame_step

        tTrack = TTrack(object.name)
        ...
        if actionFcurves:
            ...
            # Local rest matrix (relative to the parent)
            restMatrix = obj.matrix_local.copy()
            if obj.parent:
                restMatrix = poseBone.parent.matrix_local.inverted() * restMatrix
        ...
        for time in range(startframe, endframe, scene.frame_step):
            ...
            if actionFcurves:
                ...
                # This matrix is referred to the armature (object space)
                poseMatrix = obj.matrix_local.copy()

                if parent:
                    poseMatrix = parent.matrix_world.inverted() * poseMatrix
            ...
            # Convert transforms to left hand:
            tl = Vector((t.x, t.y, -t.z))
            ql = Quaternion((q.w, -q.x, -q.y, q.z))
            sl = Vector((s.x, s.y, s.z))
    ...
    # Restore initial action and frame
    obj.animation_data.action = savedAction
    obj.animation_data.use_nla = savedUseNla
    scene.frame_set(savedFrame)
#END ==================================
...
def Scan(context, tDataList, errorsMem, tOptions):
    ...
    for obj, lodName, lodDistance in meshes:
        ...
        if tOptions.doBones:
            ...
            if armatureObj:
                ...
                if not tData.bonesMap or not tOptions.mergeObjects:
                    ...
                    if tOptions.doBones: DecomposeArmature(scene, armatureObj, obj, tData, tOptions)
                if tOptions.doBones and tOptions.doAnimations and (not tData.animationsList or not tOptions.mergeObjects):
        ...
        #START =============================
        # Decompose node animations
        if tOptions.doNodeAnim:
            DecomposeNodeActions(scene, obj, tData, tOptions)
            obj.urho_create_subnode = True
        #END =============================
