...
from .export_scene import UrhoScene, UrhoWriteMaterial, UrhoWriteMaterialsList
from .prefabs import SOptions, UrhoExportPrefabs, UrhoComponentsPanel, UrhoComponentModelResetOperator, UrhoComponentShapeResetOperator, UrhoComponentBodyResetOperator
...
from bpy.types import AddonPreferences, Material, Object, Operator, Panel, PropertyGroup, Scene
...
class UrhoAddonPreferences(AddonPreferences):
    bl_idname = __name__
...
class UrhoExportSettings(PropertyGroup):
    ...
    def update_func(self, context):
        ...
        # Morphs, Materials and Prefabs need geometries
    ...
    def reset(self, context):
        ...
        self.skyboxPath = "Materials/Skybox.xml"
    ...
    nodeAnimations = BoolProperty(
            name = "Node Animations",
            description = "Export node animations (transforms). It inherits settings from Skeletons > Animations above, so you will have to (temporarily) enable these options to access them",
            default = False)
    ...
    physics = BoolProperty(
            ...
            description = "Generate physics RigidBody(s) & CollisionShape(s)",
            default = False)

    batchComponents = BoolProperty(
            name = "Propagate active object components to all",
            description = "Apply the components settings from the active object (last selected) to every exported objects ('batch' settings)",
            default = False)

    navigation = BoolProperty(
            name = "Navigable Component for root",
            description = "Add Navigable component to the root node (Note: Navigable is recursive by default)",
            default = False)

    createSkybox = BoolProperty(
            name = "Skybox",
            default = False)

    skyboxPath = StringProperty(
            name = "Path to xml material file",
            description = "Note: path is relative to Urho3D 'data' folder.",
...
class UrhoExportResetOperator(Operator):
...
# View log button
class UrhoReportDialog(Operator):
    """ View export log """
...
# Export button
class UrhoExportOperator(Operator):
    """ Start exporting """
...
class UrhoExportRenderPanel(Panel):
    ...
    def draw(self, context):
        ...
        row = box.row()
        row.prop(settings, "nodeAnimations")
        row.label("", icon='SHAPEKEY_DATA')
        ...
        if settings.prefabs:
            ...
            if not settings.merge:
                row = box.row()
                row.separator()
                row.prop(settings, "batchComponents")
                row.label("", icon='PASTEDOWN')
            #END ============================

            row = box.row()
            row.separator()
            row.prop(settings, "navigation")
            row.label("", icon='MOD_LATTICE')

            if settings.scenePrefab:
                row = box.row()
                row.separator()
                row.prop(settings, "createSkybox")
                row.label("", icon='IMAGE_COL')

                if settings.createSkybox:
                    row = box.row()
                    row.separator()
                    row.prop(settings, "skyboxPath")
...
def unregister():
    ...
    bpy.utils.unregister_class(UrhoComponentsPanel)
    bpy.utils.unregister_class(UrhoComponentModelResetOperator)
    bpy.utils.unregister_class(UrhoComponentShapeResetOperator)
    bpy.utils.unregister_class(UrhoComponentBodyResetOperator)

    del Scene.urho_exportsettings
...
def ExecuteUrhoExport(context):
    ...
    tOptions.doNodeAnim = settings.nodeAnimations
    ...
    sOptions.batchComponents = settings.batchComponents
    sOptions.navigation = settings.navigation
    sOptions.createSkybox = settings.createSkybox
    sOptions.skyboxPath = settings.skyboxPath
    ...
    for tData in tDataList:
        ...
        if settings.materials and settings.geometries:
            for uMaterial in uExportData.materials:
    ...
    if settings.prefabs and settings.geometries:
        log.info("---- Generating Prefabs ----")
        if settings.individualPrefab:
            UrhoExportPrefabs(context, uScene, sOptions, fOptions, tOptions, 0) # Do a pass '0' for individual
        if settings.collectivePrefab and not settings.merge:
            UrhoExportPrefabs(context, uScene, sOptions, fOptions, tOptions, 1) # Do a pass '1' for collective only when not merging (as it equates to an individualPrefab)
        if settings.scenePrefab:
            UrhoExportPrefabs(context, uScene, sOptions, fOptions, tOptions, 2) # Do a pass '2' for scene
